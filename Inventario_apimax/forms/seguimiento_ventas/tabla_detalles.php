...
try {
	...
	while ($row_ventas = $consulta->fetch(PDO::FETCH_NUM)) {
		...
		if ($row_ventas[5] == "1") {
			$estado_pago = "<a href='#' class='btn btn-info' title='Estado' onclick='cambiar_estado($row_ventas[0], $row_ventas[5], $row_ventas[1]);' class='btn btn-success' title='Estado'>Pagado</a>";
		} else {
			$estado_pago = "<a href='#' class='btn btn-info' title='Estado' onclick='cambiar_estado($row_ventas[0], $row_ventas[5], $row_ventas[1]);' class='btn btn-danger' title='Estado'>No pagado</a>";
		}
